import {createBarbers,getBarbers,deleteBarbers,getBarber,updateBarbers,loginBarber} from '../controllers/barbers.js'
import authorize from '../middlewares/verifyAccess.js'
import {verifyToken} from '../middlewares/verifyJwt.js'

import verifyRatelimiter from '../middlewares/verifyRatelimiter.js'
import {globalLimiter,registerLimiter, loginLimiter} from '../services/rateLimiter.js'

import express from 'express'
const routerBarbers = express.Router()


routerBarbers.get('/validate',verifyRatelimiter(globalLimiter), verifyToken, async (req, res) => {
...
routerBarbers.get('/',  verifyRatelimiter(globalLimiter), authorize(['barbeiros']),async (req, res) => {
...
routerBarbers.get('/:id',verifyRatelimiter(globalLimiter) ,async (req, res) => {
...
routerBarbers.post('/create', verifyRatelimiter(registerLimiter),async (req, res ) =>{
    try {
...
routerBarbers.delete('/:id', verifyRatelimiter(globalLimiter),authorize(['barbeiros']), async (req, res ) =>{
    const id = req.params.id
...
routerBarbers.patch('/:id', verifyRatelimiter(globalLimiter),authorize(['barbeiros']), async (req, res ) =>{
    const id = req.params.id
...
// ROTA DE LOGIN
routerBarbers.post('/login', verifyRatelimiter(globalLimiter), async (req,res) =>{
    try{
