import {tabela_barbearia, tabela_barbeiros,login_barbearia} from '../config/tables.js'
...
export async function loginBarber(dados){
    try{
        const [user] = await db.select().from(login_barbearia).where(eq(login_barbearia.email, dados.email))
